...
Every number here is produced by the `benchmark` workflow in the repository, on a GitHub-hosted runner, against hev-socks5-tunnel, sing-box and tun2socks over the same SOCKS5 server. Nothing is measured on a developer machine.
...
Locally, with the peer engines built and unprivileged namespaces allowed:

```sh
SOCKS_SERVER=/path/to/hev-socks5-server HEV=/path/to/hev-socks5-tunnel \
  DURATION=10 REPEAT=3 sh scripts/bench_netns.sh
python3 scripts/bench_charts.py bench-results docs/bench
```
